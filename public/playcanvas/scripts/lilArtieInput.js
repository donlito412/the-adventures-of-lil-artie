...
LilArtieInput.prototype.initialize = function () {
    ...
    this.keyboardRunHeld = false;
    this.gamepadRunHeld = false;
    this.runHeld = false;
...
LilArtieInput.prototype.applyDeadzone = function (value) {
...
LilArtieInput.prototype.update = function () {
    ...
    for (var i = 0; i < pads.length; i++) {
        ...
        var lx = this.applyDeadzone(pad.axes[0] || 0);
        var ly = this.applyDeadzone(pad.axes[1] || 0);
        var rx = this.applyDeadzone(pad.axes[2] || 0);
        var ry = this.applyDeadzone(pad.axes[3] || 0);
        var hasMove = Math.abs(lx) > 0 || Math.abs(ly) > 0;
        var hasLook = Math.abs(rx) > 0 || Math.abs(ry) > 0;

        if (hasMove || hasLook) {
            this.lastDevice = pad.id && pad.id.indexOf('Wireless Controller') !== -1 ? 'playstation' : 'xbox';
        }

        if (hasMove && !usingKeyboardMove) {
            x = lx;
            z = ly;
        }
        ...
        this.gamepadRunHeld = this.gamepadRunHeld || !!(pad.buttons[10] && pad.buttons[10].pressed);
    }
    ...
    this.look.set(lookX, lookY);
    this.runHeld = this.keyboardRunHeld || this.gamepadRunHeld;
};
